...
module.exports = async(req,res) =>{
    ...
    const puuid = axios.get(`https://kr.api.riotgames.com/tft/summoner/v1/summoners/
    by-name/${summoner_name}
    ?api_key=${api}`).then(res => res.body.puuid)
    ...
    const matchHistory = gameData.map(x => 
        axios.get(`https://asia.api.riotgames.com/tft/match/v1/matches/
        ${x}?api_key=${api}`).then(
            res =>{
